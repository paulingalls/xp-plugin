...
def _commands(raw: str, owner: str, runnable: bool, chained: bool) -> list[list[str]]:
    ...
    while i < len(raw):
        ...
        if char in "'\"":
            ...
            if chained and char == "&" and raw[i : i + 2] == "&&":
                ...
                raise ValueError(f"refused: {owner} contains shell syntax {char!r}")
        i += 1
    ...
    if runnable and (missing := next((a[0] for a in commands if not shutil.which(a[0])), "")):
        raise ValueError(f"refused: {owner} command {missing!r} is not runnable on this PATH")
    return commands


def verify_commands(story_id: str, card: str, runnable: bool = True) -> tuple[str, list[list[str]]]:
    raw = next(
        (ln[7:].strip() for ln in card.splitlines() if ln.startswith("Verify:")),
        None,
    )
    if raw is None:
    ...
    if not raw:
        raise ValueError(
            f"refused: {story_id}'s Verify: line is empty — put its commands on the SAME line"
        )
    return raw, _commands(raw, f"{story_id}'s Verify: line", runnable, True)
...
def run(command: str, event: str, identity: str) -> str:
    ...
    try:
        argv = _commands(command, owner, False, False)[0] + [event, identity]
