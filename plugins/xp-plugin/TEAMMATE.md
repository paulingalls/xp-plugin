...
- **Multi-file change?** Draft the plan to a file, then review it headlessly:
  `python3 {PLUGIN_ROOT}/scripts/plan_review.py <story-id> <plan-file>`. It takes
  minutes and detaches itself — a call cut short loses nothing, run it again to
  rejoin. No findings, no review: never proceed.
