...
1. **Preflight**: clean tree, on the story branch.
2. **Read the round `spawn` already recorded** — the reviewer was its fourth stage.
   Run `close.py story <id> review` only if the tree moved since; by reflex it re-reviews a reviewed diff. The leg FIXES what it finds and
   commits: reading that diff is your judgment point, running land accepts it.
...
4. **`close.py story <id> land`** — deterministic, and it never spawns. Run it from
   ...
   YOUR SHELL IS LEFT IN A DELETED DIRECTORY.
