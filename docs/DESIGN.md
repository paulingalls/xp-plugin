...
- **And no subagents, which costs one PROCESS step, not just a banner.** TEAMMATE.md — inlined verbatim on both legs — tells a teammate to spawn the `plan-reviewer` before any multi-file change. With no `--plugin-dir`, `agents/plan-reviewer.md` is not even present on the Codex leg, so that step has no mechanism: the teammate skips it or invents it. Until a Codex route exists, the lead owns plan review for a Codex-executed story.
