...
**Branching**: `release: sprint` (default) records a per-clone sprint branch; stories merge there and successful post-merge clears it. Its PR lands when the batch is releasable, normally sprint close; prefer flags/config that dark-launch unready behavior over carrying it (past ~2 sprints is drift risk). `release: story` releases per story. **Main only moves by release**: every merge to main bumps and tags. Between-sprint tweaks ride a free branch — a story branch without a card (reviewer on the diff, PR) — and a patch release.
