...
- **The Codex sandbox is project-selectable; Claude is untouched.** `codex_sandbox` accepts `workspace-write` and `danger-full-access`, defaulting to the latter so a project with no key gets byte-identical v0.7.1 argv. Every Codex caller resolves it before launch; `teammate_tee.sandbox_line` prints the posture read back off that argv. `read-only` is refused because every launched role must write its deliverable to the worktree or data root. **Measured 0.149.0** (walk: `<data-root>/walks/free-2026-08-24-codex/`, three arms with a control): under `workspace-write` the docker socket, loopback TCP and a nested `codex exec` are denied; `danger-full-access` lifts all three, while `--add-dir` grants path writes rather than socket-connect capability. The executor's linked-worktree git common dir is added separately so workspace-write can commit. This key is deliberately Codex-only; story-040 still owns any cross-harness posture word. History: v0.7.1 made every Codex leg danger-full-access, matching Claude's lack of OS confinement but leaving no opt-out.
...
The sub-allocation has been re-cut thirteen times and the total never has: `test_ratchet` asserts the four sum to ≤ 5,000, so every move is mechanically zero-sum and priced against MEASURED occupancy, never a reserve. The thirteenth (2026-08-25, Codex posture opt-out) moved 120 spawn + 55 hooks to 115 close + 60 misc. Measured before the move: spawn 1,132/1,345, close 1,979/1,980, hooks 510/590 and misc 1,034/1,085; the new caps leave 93, 116, 25 and 111 lines respectively. Hooks is the standing source because constraint 7 keeps it deterministic and judgment-free. Spawn's Codex reserve funded measured sandbox/install/tier surface and is now spent surface rather than anticipated, the same reason the eleventh move sourced it. The twelfth moved 60 hooks→misc for story-041; the eleventh moved 50 spawn→misc for story-031. Rationale for the earlier ten lives in git — `git log -p docs/DESIGN.md` over this paragraph is the ledger.
