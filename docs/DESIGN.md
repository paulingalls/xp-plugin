...
The 5k-line total is **sub-budgeted**, because the two surviving big components already threaten it (the predecessor's spawn subsystem alone is ~1.7k+ lines, and the Codex leg's measured requirements — sandbox flags, install preflight, catalog re-enumeration, tier handling — are feature surface, not prose): spawn CLI ≤ 1,225 · close component ≤ 2,095 · hooks + both harness adapters ≤ 535 · scaffolding/validators/misc ≤ 1,145.

The sub-allocation has been re-cut thirteen times and the total never has: `test_ratchet` asserts the four sum to ≤ 5,000, so every move is mechanically zero-sum, and each was priced against a MEASURED occupancy at the moment it was made — never against a reserve. Nine of the ten fed the close component, which is what a fixing reviewer, an authorship gate, an abort/undo path, an overlap check and the sprint pipeline all land in. The tenth (2026-08-23, Paul, at the v0.6.3 trim) is the first to correct an over-allocation rather than fund a feature: hooks was given 1,000 lines when the hook layer was expected to grow, and five sprints later it measures 416 — constraint 7 is why it will not grow, since hooks must stay deterministic Python with no judgment in them. 200 moved hooks→close and 150 hooks→misc, leaving hooks 650 against the one queued hook-side candidate. The two components every story touches had 24 and 10 lines of headroom at the time; they now have 244 and 156. The twelfth (2026-08-24, closing sprint-006's second pair) moved 60 hooks→misc for story-041, which touches plan_review.py and work.py — both misc, which measured 1,016 of 1,025 after 031 and 032 landed. Nine lines. Source is hooks for the same reason the tenth move used it: constraint 7 keeps hooks deterministic with no judgment in them, so its 212 idle lines are the one reserve not waiting on a feature. The thirteenth (2026-08-25, v0.7.2, opening sprint-007) is the largest and the only one cut for a WHOLE SPRINT rather than a story: 120 spawn→close, 55 hooks→close and misc, leaving spawn 1,225 · close 2,095 · hooks 535 · misc 1,145. Close measured 1,979 of 1,980 — ONE line — with three of the sprint's six cards charging to it, so the alternative was three separate mid-sprint moves, each landing on trunk ahead of a review round. Priced against measured occupancy as always: hooks is the standing reserve under constraint 7, and spawn's Codex-leg allowance is spent surface now rather than anticipated, which is the same argument moves ten and eleven made. This repo's rule that a cap move reaches trunk BEFORE any review round it must cover is why it rides a patch release instead of the sprint it funds. The eleventh moved 50 spawn→misc so story-031 could absorb the plan-review-edits-in-line change: `plan_review` is in no named set and so falls to misc, which measured 968 of 975 — seven lines. Priced against misc's need, not spawn's slack: spawn measured 974 of 1,395, and the reserve it was given for the Codex leg's sandbox flags, install preflight and tier handling is largely spent surface now rather than anticipated. Rationale for the earlier nine lives in git — `git log -p docs/DESIGN.md` over the sentence above is the ledger, and the count came from it rather than from the ordinals this paragraph replaced, which began at sprint-004 and so read three short.
