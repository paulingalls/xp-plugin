...
2. **Collision-report value measurement — MEASURED 2026-08-29, and the answer is no.** `report_merge` has NEVER FIRED: `<data-root>/reports/merge/` does not exist. Not because the reader ignored the signal, but because it was never offered one — card review already serialises stories that share files (story-066's own card: "slot 2 of 3, after 067 because both touch close/land.py"), so the condition the emitter exists to report is prevented upstream. Waiting another sprint would measure branch topology, not reader behaviour. No sprint-reviewer charter or stage angle mentions the bundle section either. RESOLUTION: keep the narrowing (a sprint-branch overlap still does not buy a second story round) and the land-time stdout print, which has a demonstrable reader because the lead redirects every close leg to a file and reads it; delete the file store and the unread bundle section.
