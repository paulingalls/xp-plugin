...
**Branching**: `release: sprint` (default) records a per-clone sprint branch; stories merge there and successful post-merge clears it. Its PR lands when the batch is releasable, normally sprint close; carrying it increases drift risk. `release: story` releases per story. **Main only moves by release**: every merge to main bumps and tags. Between-sprint tweaks use a reviewed free branch and patch release.
