...
**Story** (solo or teammate): red → green → refactor, commit small via git hooks. At story close, **close.py spawns the story reviewer itself** — fresh context, cumulative story diff plus the work.md entries filed during the story (correctness self-find, fault-injection on every new guard *and* on the filed falsifiers, scope honesty) — then stops for the fix-or-ask judgment, runs `Verify:` + story-tier tests, and merges. The reviewer's verdict and finding count are written into the **PR body / merge-commit trailer**: there is no review marker to forge — the pipeline doesn't proceed without a live reviewer result in hand, and the evidence is git-versioned where the audit trail outlives every runtime file. If the merge is not clean, or files outside the reviewed diff changed between review and merge, close.py re-runs the reviewer on the post-resolution diff before completing — conflict resolutions are the one diff that must never ship unreviewed. One review per story — the per-commit cadence option is gone.
