...
- The **spawn CLI survives largely as-is in role** (worktree creation, clean branch, bootstrap from `system.md`, plugin preflight), while close-time bookkeeping runs teardown before removal — these are the pieces neither harness provides natively. Resume distinguishes no tree/marker as never spawned, a tree without a marker as RUNNING, and explicit STOPPED/FINISHED handbacks; FINISHED remains valid only while clean.
