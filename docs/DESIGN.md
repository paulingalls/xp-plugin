...
never reads env.json.** A consumer without `plugins/xp-plugin` keeps the invoked installed
root and unchanged output. When the integration target carries that path, spawn
substitutes its future worktree root into the executor's commands and tells the lead to
use that root and manifest version for every review and land leg.
...
The 5,500-line total is **sub-budgeted**, because the two surviving big components already threaten it (the predecessor's spawn subsystem alone is ~1.7k+ lines, and the Codex leg's measured requirements — sandbox flags, install preflight, catalog re-enumeration, tier handling — are feature surface, not prose): spawn CLI ≤ 1,455 · close component ≤ 2,325 · hooks + both harness adapters ≤ 545 · scaffolding/validators/misc ≤ 1,175.

The total moved once, from 5,000 to 5,500 (2026-08-25, Paul), after a three-agent read-only audit found only ~65 removable lines among 4,815 shipped (1.4%), about six genuinely dead; ~50 of the 65 are byproducts of refactors that fix divergent behavior, so spending those savings is roughly line-neutral. This remains an 89.5% cut from the predecessor's ~52,500 and the strictest cut in the table, above the hard 80% floor. **The seventeenth re-cut is 1,455/2,325/545/1,175** (2026-08-26, Paul, at Sprint 8's open): 20 spawn + 40 hooks -> close, total unmoved. Taken AT PLANNING rather than mid-story, which is the change this one records: close measured 2,265/2,265 — zero free — while every close-pipeline card in the Sprint-8 pool charges to it, and two notes (708ab042, 6a8b38d7) had already recorded correct fixes going unshipped because a component at its cap was choosing them. hooks funds the larger share because no scheduled card touches session_start, stop_gate or bash_status. The sixteenth was 1,475/2,265/585/1,175 (at story-043's land): 20 spawn -> close, total unmoved. close measured 2,245/2,245 — flush, zero free — while spawn sat at 1,329/1,495 with 166 idle, and story-043 needed six lines close did not have. The sprint-007 card had forbidden a second rebalance on the premise that close held 116 free for three cards; it held 40, and three cards drew on it, so the premise was wrong rather than the rule. Taken as a re-cut rather than a deletion because note 608f71da binds it: a leg that meets a wall mid-story does the cheapest thing that makes the wall go away, and constraint 1's displacement is not the implementer's to choose under pressure. The fifteenth was 1,495/2,245/585/1,175: +200 spawn and +150 close price the measured sprint shortfalls rather than a reserve; +50 hooks funds the shared failure helper even though that refactor returns lines; +100 misc funds that helper's home and plan review. `test_subbudgets_sum_to_total` now requires exact equality, so a future total raise cannot create headroom no component can spend. Rationale for the earlier fourteen re-cuts lives in git — `git log -p docs/DESIGN.md` over this paragraph is the ledger.
