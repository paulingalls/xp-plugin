...
├── archive.md                     OPTIONAL, and nothing writes it: a triage decision appends an `## archived` block to work.md instead, so an archived debt's falsifier keeps running because the block never moves. corpus() still READS archive.md for a consuming project that keeps one.
...
| **debt** | claim + falsifier command (currently green) + files | Lives only until next sprint planning: scheduled under the debt budget **or dropped**. Drop ≠ delete: `work.py archive --ref <id> --disposition <text>` records the decision in place, and sprint close batch-runs archived falsifiers — they never left work.md, so this costs no move — one that reds is re-filed as a bug; untouched for N sprints, it purges. This is what makes "a dropped debt that matters will red again" *true* for the silent-cumulative class (unbounded spend, orphaned resources) whose falsifiers live outside every test tier. |
