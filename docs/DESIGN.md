...
The sub-allocation has been re-cut seven times and the total never has: `test_ratchet` asserts the four sum to ≤ 5,000, so every move is mechanically zero-sum, and each was priced against a MEASURED occupancy at the moment it was made — never against a reserve. Six of the seven fed the close component, which is what a fixing reviewer, an authorship gate, an abort/undo path, an overlap check and the sprint pipeline all land in. The seventh (2026-08-23, Paul, at the v0.6.3 trim) is the first to correct an over-allocation rather than fund a feature: hooks was given 1,000 lines when the hook layer was expected to grow, and five sprints later it measures 416 — constraint 7 is why it will not grow, since hooks must stay deterministic Python with no judgment in them. 200 moved hooks→close and 150 hooks→misc, leaving hooks 650 against the one queued hook-side candidate. The two components every story touches had 24 and 10 lines of headroom at the time; they now have 244 and 156. Rationale for the earlier six lives in git, where the moves are.
