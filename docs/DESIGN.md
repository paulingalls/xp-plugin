...
| Codex filters what its shell inherits: `shell_environment_policy.inherit` is `core \| all \| none` (measured 0.147.0 — the enum is in the error text for a bad value), a `~/.codex/config.toml` key | Our two hardest gates are ENVIRONMENT variables — `XP_ROLE` (close.py refuses a non-lead, and an ABSENT value reads as `lead`) and `GIT_AUTHOR_*`/`GIT_COMMITTER_*` (what `check_reviewer_motion` reads to tell the reviewer's fixes from the lead's). On Claude they arrive by plain process inheritance; on Codex a user's config file decides. So **`-c shell_environment_policy.inherit=all` rides every Codex argv** — unpinned, a teammate self-closes and a reviewer's commits are signed by the lead |
