...
├── plan.md                        roadmap: milestones + current sprint stories — PER CLONE
...
| **resolved** | prior id + replacement falsifier **green right now** + explicit covered tier or `none` | Sprint-002 amendment. Records ARE named — by `sha256(entry)[:8]`, derived not stored, because an ISO second is not a name (measured: 48 concurrent appends, 48 identical headings) and an append-only file cannot be backfilled. The sprint-close batch checks every record no archive has removed — a resolution substitutes its falsifier rather than dropping one, so a RESOLVED record still runs; archived ones left the batch at story-051, see the debt row — and a stale bug falsifier is replaced rather than wedging the close. Resolution SUBSTITUTES a falsifier rather than deleting one: an unchecked done marker could silence a live bug, whereas a wrong replacement reds later and reopens it. Coverage belongs to the replacement and is never inherited. **Polarity**: a debt's falsifier asserts the system is still OK, so red means the latent problem materialised — one that greens *because* the flaw is present is inverted. |
...
`--covered-by TIER` is the project's declaration that a record's opaque command selects work in that configured tier. Sprint close defers it when that tier's command equals the running command, or when a generic `tier_coverage` graph says the running tier covers it. The graph pins every participating command; missing tiers, cycles and stale pins refuse before execution. A red covering tier runs deferred falsifiers individually. The plugin neither parses runner output nor infers selection relationships. Standalone timings stay ephemeral; bounded, cost-sorted archive offers state that disposal forfeits the replacement guarantee. Runner introspection, batching, automatic archive and a refusal ceiling remain deliberately absent.
...
- **Sprint cap** (default ~6 stories) — slate review counts consumed capacity; work already shipped in a free patch consumes no slot. Smaller sprints make the test tiers work and stop the doubling; debt budget is a share of the cap.
...
The gap that leaves — a session dying mid-story — is covered mechanically at the next start. The banner's `recover` command starts with one deterministic `NEXT:` line from the card headings of the sprint the state root records as OPEN — falling back to the highest-numbered heading, loudly, when nothing is recorded (story-122) — and story-scoped worktree/handoff state: `[planned]` → `spawn.py ready`, `[ready]` → spawn, `STOPPED` → resume, `FINISHED` → story close, and no open card → sprint close. Missing worktrees name recovery; `RUNNING`, parallel in-progress cards and malformed or missing state refuse to guess. It then assembles current branch, dirt, cards, last close and work.md titles beside the digest and sprint slice; PROCESS runs it first. A written-at + git-HEAD stamp marks a moved digest stale. The lead is `session.md`'s sole writer; artifacts win on conflict. Staleness is handled ONLY here — the Stop nudge §7 once carried was removed for firing every turn rather than at session end.
...
**Slate review** is the first of four review points. At sprint creation the lead on either harness runs `slate_review.py`, whose fresh `slate-reviewer` receives the slate, capacity, VALUES, JUDGMENT, constraints and system context, never the author's conclusions, and returns an absolute findings path. Its scoped incomplete marker precedes launch and only the accepted child removes it, so the marker's absence is success. The lead checks every claim, records accepted and rejected conclusions in work.md, and corrects cards only; creation owns order, funding, collisions and dependencies. At Sprint 15 all five earned; archive-copy mutation, the suite and omitted-pin searches became checks. Sprint 24 retired mutation (Paul): building each card in a copy cost 29 minutes a round, and three of its four findings were reachable by reading. One false citation keeps the lead judging every result.

Timing differs: note f56d5290 found three needed the later tree and four needed the whole slate, one in both. **Execution plan review**, the second point, gives multi-file work current-tree pressure through `plan_review.py` AFTER `spawn.py ready`; reports are `<data-root>/plans/<story-id>.round-N.md`, numbered from `<story-id>.round-1.md`; the legacy round-one name `<data-root>/plans/<story-id>.md` is still read and labelled, never allocated again. It stays advisory: after two infrastructure failures TEAMMATE routes the executor to a `work.py note`; a hard wall still owes a close refusal and distinct-state boundary tests. Ready remains a mint, not a review credential, so a single-file card gets none and a skipped review leaves no record. Story and sprint close use `close.py`; headless runners keep every point dual-harness.

**Plan findings handoff**: after a multi-file review, spawn names the current disposition in the launched executor prompt, including resume. The executor reads it, runs reviewed-plan diagnostics, and uses `work.py card-snapshot` plus locked `edit-card` to add touched Files and repeatable checks to Verify. The reviewer remains plan-only; AC and scope changes belong to the lead.

**Card refresh** runs between slate review and execution plan review, and is explicitly not a review: `slate_review.py --refresh <story-id>` launches a fresh `card-refresher` through the same detached lifecycle to correct one card's stale existing-code claims against HEAD, never repository or sibling-card motion — the lead owns the rewrite, not a verdict; a receipt binds it to HEAD: `<data-root>/card-refreshes/<story-id>.json` stores HEAD, the post-refresh card digest, whether the card changed, and each `Files:` path's committed state — a SHA, or explicit null while a to-be-created path stays absent. `spawn.py ready` refuses before minting unless the receipt was taken against exactly this card's text and every declared path's committed state is unchanged since — it compares those paths, never HEAD itself, so an unrelated commit does not stale a refresh; the FREE LANE is exempt — `close/free.py`'s internal mint and `spawn.py ready` on a free key alike, so one operation does not answer two ways depending on which leg reached it — because a free card is authored and reviewed on its own just-cut branch and is never aged in a slate.
...
| Teammate | VALUES + JUDGMENT + `TEAMMATE.md` (one page: story scope, plan ownership, escalation, Verify handback) + its story card + constraints.md | the profile reports the plugin's actual share of the composed total, measured over the prompt-carried plugin prose alone — `templates/constraints.md` ships in the package but never reaches a teammate, so counting it overstated the plugin by a third. `profile_target` is the advisory story-card allowance, default 806 tokens for a missing legacy key: 4,500 minus the largest measured project floor, 3,694. Excluding that project-specific floor gives each lead the same card allowance; the composed total remains reported, never capped. |
| Plan / story reviewer | VALUES + JUDGMENT + its review charter + plan or pinned cumulative-diff range/read command/bounded metadata + card + constraints.md + **system.md**; story review also receives story work records | < 2.5k tokens; neither receives PROCESS |
| Sprint finder / verifier / fixer / closer | VALUES + JUDGMENT + its one stage charter + pinned sprint-diff range/read command/bounded metadata + cards/work/resolutions + constraints.md + **system.md**; finders also receive one angle | PROCESS absent; the fixer receives the canonical comment rubric before editing |
...
## 9. Size build record (the 80–90% cut)

Frozen at the v0.18.1 close on 2026-09-03. This section records the build's
targets, measurements, and re-cuts; it is not runtime configuration or current
acceptance criteria. Live component and density guidelines exist only in the
repository report that prints them.
...
These were acceptance criteria while the plugin was built. At this record's
freeze, tests were 3.51 times shipped code, near the predecessor's 3.6 ratio and
well beyond the original two-times target; unlike the component arithmetic,
that ratio had not been enforced.

The build also tracked comments and docstrings across shipped Python. The
predecessor reached 33% with no counter-pressure. JUDGMENT.md became the shipped
copy of the comment rubric and finding bar; reviewer charters point at it and
every reviewer bundle carries it. The recorded cap moves below show what that
numeric policy cost and why it was retired.

**THE PROSE CEILINGS GOT A FLOOR** (2026-09-03, Paul, at free-2026-09-03-prose-sufficiency).
...
RULE THEN: every purchasable prose cap was at least live / 0.9, so at least 10% of the cap
...
PROCESS.md was also given a local cap after bisection against that byte profile.
That cap was later retired: the real boundary covers the composed injection, not
one neighbouring document, and the byte-profile wall already exercises it.

The build's 6,150-line total was **sub-budgeted**, because the two surviving big components already threatened it (the predecessor's spawn subsystem alone is ~1.7k+ lines, and the Codex leg's measured requirements — sandbox flags, install preflight, catalog re-enumeration, tier handling — are feature surface, not prose): The twenty-fifth re-cut is **1,480/2,625/665/1,380** (2026-09-03, Paul, at story-102's close) — 20 more misc -> spawn, total unmoved at 6,150, and the SECOND re-cut in one day, which DESIGN's own ledger warns against. Measured at b1605cf: story-102 consumed the twenty-fourth re-cut EXACTLY, leaving spawn 1,460/1,460 — flush, zero free — while story-103 still owes `spawn` a ready.py refusal. Paul asked for a displacement to be sought FIRST: an AST scan over the whole component found no unused module-level function, and story-102 had already shed docstrings from teammate_tee.py to fit, so there was nothing to remove that a reader would not miss. misc funds it again at 1,348/1,380, 32 free. THE LEDGER'S OWN WARNING STANDS AND IS NOT DISCHARGED BY THIS ENTRY: two re-cuts in a day is the pattern the note calls measuring the wrong invariant. The honest reading is that Milestone 8 moves work INTO spawn by design — four staged roles where there was one teammate — so the component's share of a fixed total is what needs re-deciding at the next planning, not its ceiling under pressure. THE TWENTY-SIXTH RE-CUT IS THE FIRST THAT MOVES THE TOTAL, 6,150 -> 6,300 (2026-09-03,
