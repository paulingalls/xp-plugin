...
| Teammate | VALUES + JUDGMENT + `TEAMMATE.md` (one page: story scope, plan ownership, escalation, Verify handback) + its story card + constraints.md | `plugin_shipped_chars` measures the package (1,670 tokens at story-119), unenforced; the profile reports the actual plugin share over its composed total. `profile_target` is the advisory story-card allowance, default 806 tokens for a missing legacy key: 4,500 minus the largest measured project floor, 3,694. Excluding that project-specific floor gives each lead the same card allowance; the composed total remains reported, never capped. |
