...
The 5,500-line total is **sub-budgeted**, because the two surviving big components already threaten it (the predecessor's spawn subsystem alone is ~1.7k+ lines, and the Codex leg's measured requirements — sandbox flags, install preflight, catalog re-enumeration, tier handling — are feature surface, not prose): spawn CLI ≤ 1,425 · close component ≤ 2,365 · hooks + both harness adapters ≤ 535 · scaffolding/validators/misc ≤ 1,175.

The total moved once, from 5,000 to 5,500 (2026-08-25, Paul), after a three-agent read-only audit found only ~65 removable lines among 4,815 shipped (1.4%), about six genuinely dead; ~50 of the 65 are byproducts of refactors that fix divergent behavior, so spending those savings is roughly line-neutral. This remains an 89.5% cut from the predecessor's ~52,500 and the strictest cut in the table, above the hard 80% floor. **The eighteenth re-cut is 1,425/2,365/535/1,175** (2026-08-27, Paul, at Sprint 9's open): 30 spawn + 10 hooks -> close, total unmoved. Measured at d473198: close 2,311/2,325 — FOURTEEN free — while both code cards on the slate charge to it (058 is net-negative there, 053 is ≤25) and the lane is ordered, so the second card would have been funded by the first one's deletion or not at all. spawn funds the larger share because no scheduled card touches it AND its headroom is partly illusory: spawn.py sits at 499 of constraint 8's 500-line file cap, so growth there means extraction before it means lines. hooks funds the rest and keeps 36 free, because sprint-8's review fixed session_start.py twice. The seventeenth was 1,455/2,325/545/1,175 (2026-08-26, Paul, at Sprint 8's open): 20 spawn + 40 hooks -> close, total unmoved. Taken AT PLANNING rather than mid-story, which is the change this one records: close measured 2,265/2,265 — zero free — while every close-pipeline card in the Sprint-8 pool charges to it, and two notes (708ab042, 6a8b38d7) had already recorded correct fixes going unshipped because a component at its cap was choosing them. hooks funds the larger share because no scheduled card touches session_start, stop_gate or bash_status. The sixteenth was 1,475/2,265/585/1,175 (at story-043's land): 20 spawn -> close, total unmoved. close measured 2,245/2,245 — flush, zero free — while spawn sat at 1,329/1,495 with 166 idle, and story-043 needed six lines close did not have. The sprint-007 card had forbidden a second rebalance on the premise that close held 116 free for three cards; it held 40, and three cards drew on it, so the premise was wrong rather than the rule. Taken as a re-cut rather than a deletion because note 608f71da binds it: a leg that meets a wall mid-story does the cheapest thing that makes the wall go away, and constraint 1's displacement is not the implementer's to choose under pressure. `test_subbudgets_sum_to_total` requires exact equality, so a future total raise cannot create headroom no component can spend. Rationale for the earlier fifteen re-cuts lives in git — `git log -p docs/DESIGN.md` over this paragraph is the ledger.
