...
- **The reviewer default is Codex.** Reviewers write a round-scoped report and unified patch without repository credentials; close.py validates `.xp/` scope, applies it, and commits under the reviewer identity. Claude uses measured `acceptEdits`; Codex gets no git-common-dir widening. Executors keep both harnesses' commit capability.
