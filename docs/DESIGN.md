...
5. **SessionStart**: role-profile injection + recovery block + enforcement banner (version, live gates). No liveness touchfile — see §3: the git-hook reader it was written for cannot distinguish this session from any other, so both halves are retired rather than shipping a writer nothing reads
