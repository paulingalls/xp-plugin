...
**Card review** (the lead's, over the sprint slate at sprint open, including its capacity) → `spawn.py ready <story-id>` mints a digest of the whole card block and flips [planned] → [ready]; it is the lead COMMITTING to this card, not a second review. A free branch resolves its dated card id at `free start`; `free review` refuses until that card exists, then uses the same mint and active-state path. **Plan review** (the executor's, over its own implementation plan for multi-file work; the LEAD NEVER DRAFTS ONE — measured twice in one week, bug 898ad9e1 and note c3d8e2a7): draft plan → **fresh-context plan reviewer** (checks TDD ordering — red before green, real-behavior-not-reachability — artifact coherence: plan vs stories vs Verify commands vs collision declarations, and constraint conflicts; edits only silent/corrupting problems into the plan with adjacent reasons, reports loud/addressable problems without another round, stops on human-only questions, and writes its disposition to `<data-root>/plans/<story-id>.md`, then `<story-id>.round-N.md`) → re-read plan and disposition → execute; spawn recomputes that digest and refuses on later card drift until `spawn.py amend` records a mandatory reason and every prior card beside the credential without changing its status; land discloses that audit trail (the bracket alone was a credential with a reader and no writer — measured three times in sprint-003). The reviewer prompt is a page, not 3,153 words.
