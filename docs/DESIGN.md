...
**Project preflight**: optional top-level `preflight` runs a seconds-scale, shell-free environment command before the expensive gates at sprint start, story/free land, and sprint land. Unquoted `&&` chains commands; other shell syntax and an executable absent from PATH refuse. Every attempt runs it, including lands whose tier receipts can be reused. Dry runs validate and preview it without execution. A red command prints its output and refuses without filing a record or receipt; successful lands report wall time and warn above 60 seconds. The working-tree config supplies the command even when land tests a trial merge. An absent or empty value does nothing.
