...
| ~~Codex cannot spawn codex on macOS~~ — **FALSIFIED on 0.149.0** (record b120edcf): both arms held the INNER invocation constant and changed the OUTER sandbox. Outer `workspace-write` reproduced `failed to initialize in-process app-server client: Operation not permitted`; outer `danger-full-access` let the inner return PONG and exit 0. story-038 then walked its real `plan_review.py` through an outer Codex executor at the shipped `danger-full-access`: the inner Codex reviewer wrote findings and exited 0 | Codex CAN spawn codex once the OUTER leg is unconfined, which is the shipped default. **Keep cross-harness plan review on its merits, not as a capability workaround**: a Codex author reviewed by Claude supplies independent model and harness pressure before code exists (constraint 3), so this repo's plan-reviewer remains `claude/opus/high`. Re-probe on Codex version bumps |
