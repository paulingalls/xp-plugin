...
- **The sandbox is asymmetric, and no stub can show it.** A Claude teammate runs unbounded in its throwaway worktree; a Codex one runs under `--sandbox workspace-write` with `--add-dir` for the data root and network off by default — **measured 0.149.0** (Sprint-5 open probe): DNS refused inside the sandbox (curl exit 6), and `-c sandbox_workspace_write.network_access=true` restores it (HTTP + exit 0). Story-026 puts that flag on the codex EXECUTOR argv only — the leg that must nest a headless plan review — so reviewer legs stay network-off and the widened executor remains strictly narrower than the unsandboxed Claude leg. A story needing an out-of-tree write still succeeds on one leg and fails on the other.
