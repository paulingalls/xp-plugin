...
├── archive.md                     dropped-debt falsifiers (see §4 — drop ≠ delete)
...
| **debt** | claim + falsifier command (currently green) + files | Lives only until next sprint planning: scheduled under the debt budget **or dropped**. Drop ≠ delete: the falsifier moves to `archive.md`, and sprint close batch-runs archived falsifiers — one that reds is re-filed as a bug; untouched for N sprints, it purges. This is what makes "a dropped debt that matters will red again" *true* for the silent-cumulative class (unbounded spend, orphaned resources) whose falsifiers live outside every test tier. |
...
**Plan** (multi-file work): draft plan → **fresh-context plan reviewer** (independent subagent; checks TDD ordering — red before green, real-behavior-not-reachability — artifact coherence: plan vs stories vs Verify commands vs collision declarations, constraint conflicts, sprint cap) → address findings → `plan-reviewed` marker set → execute. The reviewer prompt is a page, not 3,153 words.

**Story** (solo or teammate): red → green → refactor, commit small via git hooks. At story close, **close.py spawns the story reviewer itself** — exactly **one reviewer**, whose charter carries both the process lenses (fault-injection on every new guard *and* filed falsifier, artifact coherence, scope honesty, constraint drift) and the correctness angles (state/lifecycle, removed behavior, cross-file + the copy, line-scan, ecosystem pitfalls, environment assumptions). Its **depth (`standard | deep`) is assigned by the plan reviewer** at plan time — never by the author, who reliably underrates the risk of their own design — and the lead may raise it, not lower it. The lead verifies findings directly (no separate adversarial-verify stage). It then stops for the fix-or-ask judgment, runs `Verify:` + story-tier tests, and merges. **Review stopping rule**: one full review always; prescription-faithful fixes with red-first tests close without re-review (the prescription was the review); re-review only for deviations, uncovered new behavior, or conflict resolutions; hard cap two rounds, then the human decides — review loops have steeply diminishing returns and the loop-breaker is judgment, not iteration. The reviewer's verdict and finding count are written into the **PR body / merge-commit trailer**: there is no review marker to forge — the pipeline doesn't proceed without a live reviewer result in hand, and the evidence is git-versioned where the audit trail outlives every runtime file. If the merge is not clean, or files outside the reviewed diff changed between review and merge, close.py re-runs the reviewer on the post-resolution diff before completing — conflict resolutions are the one diff that must never ship unreviewed. One review per story — the per-commit cadence option is gone.

**Branching** (from xp-agents' doctrine, taxonomy dropped): `release: sprint` (default) — a sprint branch off main; stories merge there via close.py; **sprint close PRs the batch to main**, making the release moment coincide with the heavyweight gates. `release: story` for projects where per-story release is right. No stage taxonomy, no migration machinery, free-session branches when someone needs them.
...
**Sprint close**: full-tier tests → **archived falsifiers batch-run** (reds re-file as bugs) → **broad review via the Workflow tool** (fan-out over the sprint diff — cross-cutting issues; `/code-review`-style fallback where Workflow is unavailable, e.g. Codex leads) → LLM security review over the cumulative diff → **retro: a short narrative (Keep/Fix, one page) + a proposed diff** to `constraints.md` / `config.yml` / test commands, reviewed like any other change. A learning that changes nothing executable or injected is not recorded. Debt triage for the next sprint happens here with the human (schedule-or-drop).
...
| Teammate | VALUES + `TEAMMATE.md` (one page: TDD loop, commit conventions, escalate-don't-guess, done = Verify green) + its story card + constraints.md | < 2k tokens |
...
The 5k-line total is **sub-budgeted now**, because honest arithmetic says the two surviving big components already threaten it (the predecessor's spawn subsystem alone is ~1.7k+ lines, and the Codex leg's measured requirements — sandbox flags, install preflight, catalog re-enumeration, tier handling — are feature surface, not prose): spawn CLI ≤ 2,000 · close.py ≤ 800 · hooks + both harness adapters ≤ 1,000 · scaffolding/validators/misc ≤ 1,200. If a sub-budget blows, the **pre-named sacrificial features** are cut first — teammate output filtering, worktree differential reporting, per-tier effort maps — never the gates or the reviewers.
...
- **Sprint 2**: spawn CLI (Claude-only) · scoped markers · exit-status-masking gate · archive.md batch runner · sprint-close pipeline + retro-as-diff.
