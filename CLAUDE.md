...
- **Slate review**: at sprint open a fresh `slate-reviewer` reads the whole slate and
  ...
  `spawn.py ready <story-id>` now runs card refresh first — `slate_review.py --refresh
  <story-id>` rewrites the card's stale claims against HEAD, not a review — and the
  lead owns the result; ready itself remains the lead's per-card commitment, not a review.
