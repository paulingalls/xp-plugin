...
- **A merge that fails no longer crashes instead of refusing.** Both checkouts in
  ...
  one crashed the same way when no separate worktree held trunk, which is what a
  consuming project without one always has. Neither is attempted where a worktree
  already holds the branch, and a checkout that does fail is now reported.
