...
2. **Fault-inject every guard.** A check that cannot red against its target defect is
   ...
   gates, tests, and filed falsifiers. The inverse breaks a gate too: normalize raw
   wall clock against a same-run control, or use a generous HANG GUARD and assert
   the event.
