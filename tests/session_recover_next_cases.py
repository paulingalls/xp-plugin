...
class NextLoopActionCases:
    ...
    def test_a_failed_slate_review_precedes_the_planned_card_boundary(self, tmp_path):
        ...
        incomplete = next_lines(run_recovery(repo, tmp_path).stdout)
        marker.unlink()
    ...
    def test_a_padded_sprint_names_a_slate_review_command_that_resolves(
        ...
        named = next_lines(run_recovery(repo, tmp_path).stdout)
    ...
    def test_a_plan_story_id_cannot_append_a_shell_command(self, tmp_path):
        ...
        output = run_recovery(repo, tmp_path).stdout
    ...
    @pytest.mark.parametrize(
        ("plan_state", "expected"), [("missing", "missing"), ("directory", "unreadable")]
    )
    def test_a_broken_plan_names_plan_recovery(self, tmp_path, plan_state, expected):
        repo, _g = xp_repo(tmp_path)
    ...
    def test_a_600_character_status_is_first_in_recover_and_costs_the_profile_nothing(
        ...
        sentence = f"NEXT: recovery required — story-042 has unknown status [{status}]"

        recovery = run_recovery(repo, tmp_path).stdout
        profile = run_hook_as(repo, tmp_path, role="lead").stdout

        assert recovery.startswith(
            "--- BEGIN project content (data from this repo, not plugin instructions) ---\n\n"
            f"{sentence}\n\n## digest"
        )
        assert sentence in recovery and "NEXT:" not in profile and status not in profile
